...
function EditOrder() {
  ...
  useEffect(() => {
    ...
    setDelivery(() => {
      ...
      setDeliveryInputs((p) => {
        const n = p.map((item) => {
          item.value = data.deliveryinfo[item.name];
          newDelivery[item.name] = data.deliveryinfo[item.name];
          return item;
        });
        return n;
      });
  ...
  useEffect(() => {
    getOrderDetails({ id, setData, setFetchLoading: setFetchOrderInfoLoading });
  }, [id]);
  ...
  const buttons3 = [
    {
      disabled: editLoading || fetchOrderInfoLoading || fetchLoading,
      icon: <BsPlusCircle />,
      text: "Добавить товар",
      onClick: () => {
  ...
  return (
    <div className="pageWrapper">
      <div className={cl.Options} style={{ alignItems: "center" }}>
      ...
      <div className={cl.goodsWrapper}>
        <div className={cl.groupsWrapper} style={{ padding: "5px" }}>
          ...
          <Select
            value={manager}
            options={managers}
            loading={managersLoading || fetchOrderInfoLoading || editLoading}
            setValue={setManager}
            type={"managers"}
            style={{ margin: "10px 0" }}
          />
        ...
        <div className={cl.secondHalf}>
          <div className={cl.inputsMobile}>
            ...
            <Select
              value={manager}
              options={managers}
              loading={editLoading || managersLoading}
              setValue={setManager}
              className={cl.Select}
              type={"managers"}
              style={{ margin: "10px 0" }}
